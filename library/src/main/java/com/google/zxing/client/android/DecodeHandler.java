...
//  ///////my code////////////
    ...
    case Surface.ROTATION_0:
    	for(int x=0; x<width; x++)
    		for(int y=0,z=height-1; y<height || z>=0; y++,z--)
    			rotatedData[height * x + z] = data[x + y * width];
    	source = activity.getCameraManager().buildLuminanceSource(rotatedData, height, width);
    	break;
   
    case Surface.ROTATION_270:
    	for(int w=0,y=width-1; w<width || y>=0; w++,y--)
    		for(int x=0,z=height-1; x<height || z>=0; x++,z--)
    			rotatedData[z * width + y] = data[x * width + w];
